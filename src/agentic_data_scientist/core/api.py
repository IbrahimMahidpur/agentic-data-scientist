...
class DataScientist:
    ...
    async def _setup_agent(self):
        ...
        if self.config.agent_type == "adk":
            ...
            # claude_code agent uses ClaudeCodeAgent which requires an Anthropic API key.
            # In this Ollama build, we redirect to the ADK agent with OllamaCodingAgent.
            logger.warning(
                "[API] claude_code agent type is not available in Ollama mode. "
                "Falling back to adk agent with OllamaCodingAgent."
            )
            self.config.agent_type = "adk"
            from agentic_data_scientist.agents.adk import create_app
            app = create_app(
                working_dir=str(self.working_dir),
                mcp_servers=self.config.mcp_servers,
